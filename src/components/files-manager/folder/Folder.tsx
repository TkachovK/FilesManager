...
import { AccessLinkProps, CreateFolderProps, FolderProps, PermissionsProps, SetFolderRootProps } from '../../../interfaces/folder'
import { cloneFolder, createFolder, deleteFolder, getAvailableFolders, getFolder, renameFolder } from '../../../services/folders'
...
const FolderComponent: React.FC<SetFolderRootProps> = ({ rootFolder, setRootFolder }) => {
  ...
  const createEditItem = async ({ name }: CreateFolderProps) => {
    ...
    if (!!folderName) {
      ...
      setRootFolder((prevObject) => ({
        ...prevObject,
        folders: [...prevObject?.folders ?? [], { id: data.id, name, parentId: rootFolder.id, action: 'creator', folders: [] }],
      }))
    }
  }
