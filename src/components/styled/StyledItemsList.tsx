...
const StyledItemsList: React.FC<ItemsListProps> = ({ items, logo, handleItemClick }) => (
  <List style={{ padding: '6rem' }}>
    {items && items?.length > 0 && items?.map((item, i) => (
      ...
      >
        <ListItemIcon style={{ minWidth: '20px', transform: 'scale(2)' }}>
          {logo}
        </ListItemIcon>
        <ListItemIcon style={{ minWidth: '20px', transform: 'scale(1)' }}>
          {item?.permissions?.some(permission => permission.action === 'creator') &&
            item?.permissions?.map(permission => permission.action === 'creator' && permission.avatar &&
              <Avatar
                alt="Creator Avatar"
        ...
        <ListItemText
          primary={
            <Typography
              variant="subtitle1"
              style={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', width: '100px', textAlign: 'center' }}
            >
